...
def match_fingerprints(img1_path, img2_path, min_keypoints=20, match_ratio_threshold=0.15):
    ...
    good_matches = [m for m in matches if m.distance < 50]  # 50 is a typical threshold for ORB

    # Calculate match ratio
    match_ratio = len(good_matches) / min(len(kp1), len(kp2))
    print(f"Keypoints1: {len(kp1)}, Keypoints2: {len(kp2)}, Good matches: {len(good_matches)}, Match ratio: {match_ratio:.2f}")
